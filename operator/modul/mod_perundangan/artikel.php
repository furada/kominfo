...
session_start();
 if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){  
  ...
  $aksi="modul/mod_perundangan/aksi_artikel.php";
  switch($_GET[act]){
  ...
  <strong>Anda berhasil</strong> menambahkan perundangan.
  </div>";}
  ...
  <button data-dismiss='alert' class='close' type='button'>x</button>
  <strong>Anda berhasil</strong> meng-update perundangan.
  </div>";}
  ...
  <button data-dismiss='alert' class='close' type='button'>x</button>
  <strong>Anda berhasil</strong> menghapus perundangan.
  </div>";}

  
  echo "
  <div class='workplace'>
  <form action='$aksi?module=perundangan&act=hapussemua' method='post'>
  ...
  <ul class='dd-list'>
  <li><a href='?module=perundangan&act=tambahartikel'><span class='isw-plus'></span> Tambahkan </a></li>
  ...
  $tampil = mysql_query("SELECT * FROM perundangan ORDER BY id_artikel DESC");}
  
  else{
  $tampil=mysql_query("SELECT * FROM perundangan 
...
$sql="select * from kategoriperundangan where id_kategoriartikel='$kat'";
...
$row=mysql_fetch_assoc($query);
  ...
  <td>$r[username]</td>
  <td width='8%'>
  <a href=?module=perundangan&act=editartikel&id=$r[id_artikel]>
  <center><img src='img/edit.png' class='ttLT'' title='Edit Perundangan'></center></a>
  </td>
   
  <td width='8%'>
  <a href=javascript:confirmdelete('$aksi?module=perundangan&act=hapus&id=$r[id_artikel]&namafile=$r[gambar]')>
  <center><img src='img/hapus.png' class='ttLT'' title='Hapus Perundangan'></center></a> 
  </td>
  ...
  $jmldata = mysql_num_rows(mysql_query("SELECT * FROM perundangan WHERE username='$_SESSION[namauser]'"));}  
  ...
  if ($_SESSION[leveluser]=='admin'){
  $tampil = mysql_query("SELECT * FROM perundangan WHERE judul LIKE '%$_GET[kata]%' ORDER BY id_artikel DESC LIMIT $posisi,$batas");}
  ...
  if ($_SESSION[leveluser]=='admin'){
  $jmldata = mysql_num_rows(mysql_query("SELECT * FROM perundangan WHERE judul LIKE '%$_GET[kata]%'"));}
  ...
  <h1>Tambahkan perundangan</h1>
  <div class='clear'></div>
  </div>
					
  <div class='block-fluid'>             
  <form method=POST action='$aksi?module=perundangan&act=input' enctype='multipart/form-data'>
   
   
  <div class='row-form'>
  <div class='span3'>Judul perundangan</div>
  ...
  <div class='row-form'>
  <div class='span3'>Sub Judul perundangan</div>
  ...
  $tampil=mysql_query("SELECT * FROM kategoriperundangan ORDER BY nama_kategoriartikel");
  while($r=mysql_fetch_array($tampil)){
  ...
  <div class='row-form'>
  <div class='span3'>Isi perundangan</div>
  <div class='span9'><textarea id='teraskreasi' name='isi_artikel'  style='height: 350px;'>
  <b>$iden[nama_website],</b> mulai isi perundangan disini..</textarea></div>
  <div class='clear'></div>
  </div>    
  ...
  <div class='row-form'>
  <a class='btn btn-danger btn-rounded' id=reset-validate-form href='?module=perundangan'>Batal</a>
  ...
  case "editartikel":
   ...
   if ($_SESSION[leveluser]=='admin'){
  $edit = mysql_query("SELECT * FROM perundangan WHERE id_artikel='$_GET[id]'");}
  
  else{
	   $edit = mysql_query("SELECT * FROM perundangan WHERE id_artikel='$_GET[id]' AND username='$_SESSION[namauser]'");}
  ...
  <div class='head'>
  <div class='isw-documents'></div>
  <h1>Edit perundangan</h1>
  <div class='clear'></div>
  </div>
					
  <div class='block-fluid'>             
  <form method=POST enctype='multipart/form-data' action=$aksi?module=perundangan&act=update>
  ...
  <div class='span3'>Kategori</div>
  <div class='span9'>        
  <select name='kategoriperundangan' id='s2_2'> ";
  $tampil=mysql_query("SELECT * FROM kategoriperundangan ORDER BY nama_kategoriartikel");
  ...
  <div class='row-form'>
  <div class='span3'>Gambar</div>
  <div class='span9'><a class='fancybox' rel='group' href='../img_perundangan/$r[gambar]'>
  <img src='../img_perundangan/small_$r[gambar]' width=200 class='ttLT'' title='Perbesar Gambar'></a></div>
  <div class='clear'></div>
  </div>    
