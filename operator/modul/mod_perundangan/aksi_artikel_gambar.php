...
if ($module=='artikel' AND $act=='hapus'){
  ...
  if ($data['gambar']!=''){
     ...
     unlink("../../../img_artikel/$_GET[namafile]");   
     unlink("../../../img_artikel/small_$_GET[namafile]");   
  }
  else{
     mysql_query("DELETE FROM artikel WHERE id_artikel='$_GET[id]'");
  }
...
// Hapus terpilih
elseif($module=='artikel' AND $act=='hapussemua'){
if(isset($_POST['cek'])){
foreach($_POST['cek'] as $cek => $num){
$data=mysql_fetch_array(mysql_query("SELECT gambar FROM artikel WHERE id_artikel=$num"));
  if ($data['gambar']!=''){
     unlink("../../../img_artikel/$data[gambar]");   
     unlink("../../../img_artikel/small_$data[gambar]");  
} 
mysql_query("DELETE FROM artikel WHERE id_artikel=$num");
  header('location:../../media.php?module='.$module.'&msg=delete');
}
} else {
  header('location:../../media.php?module='.$module.'&msg=delete');
...
if(isset($_POST['kirimnewsletter']) && $_POST['kirimnewsletter']=='ya'){
  ...
  header('location:../../media.php?module='.$module.'&msg=update');
    ...
    $data_gambar = mysql_query("SELECT gambar FROM artikel WHERE id_artikel='$_POST[id]'");
	$r    	= mysql_fetch_array($data_gambar);
	@unlink('../../../img_artikel/'.$r['gambar']);
	@unlink('../../../img_artikel/'.'small_'.$r['gambar']);
    Uploadartikel($nama_file_unik,'../../../img_artikel/',300,120);
	watermark_image('../../../img_artikel/'.$nama_file_unik);
