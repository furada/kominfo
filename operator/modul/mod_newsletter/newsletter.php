...
session_start();
  ...
  $tampil = mysql_query("SELECT * FROM newsletter ORDER BY id DESC");}
  
  else{
  $tampil=mysql_query("SELECT * FROM newsletter 
  ...
  <center><img src='img/edit.png' class='ttLT'' title='Edit User newsletter'></center></a>
  </td>
   
  <td width='8%'>
  <a href=javascript:confirmdelete('$aksi?module=newsletter&act=hapus&id=$r[id]')>
  <center><img src='img/hapus.png' class='ttLT'' title='Hapus User newsletter'></center></a> 
  </td>
  ...
  case "editnewsletter":
  $edit=mysql_query("SELECT * FROM newsletter WHERE id='$_GET[id]'");
