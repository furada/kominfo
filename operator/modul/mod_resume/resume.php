...
session_start();
 if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){  
  ...
  $aksi="modul/mod_resume/aksi_download.php";
  switch($_GET[act]){
  default:
  ...
  <strong>Anda berhasil</strong> menambahkan Resume.
  </div>";}
  ...
  <button data-dismiss='alert' class='close' type='button'>x</button>
  <strong>Anda berhasil</strong> meng-update Resume.
  </div>";}
  ...
  <button data-dismiss='alert' class='close' type='button'>x</button>
  <strong>Anda berhasil</strong> menghapus Resume.
  </div>";}  


   echo "
  <div class='workplace'>
  <form action='$aksi?module=resume&act=hapussemua' method='post'>
  ...
  <ul class='dd-list'>
  <li><a href='?module=resume&act=tambahdownload'><span class='isw-plus'></span> Tambahkan File</a></li>
  ...
  $tampil = mysql_query("SELECT * FROM resume ORDER BY id_download DESC");}
  
  else{
  $tampil=mysql_query("SELECT * FROM resume 
  ...
  <td>$tgl</td>
    <td>$r[username]</td> 
  <td width='8%'>
  <a href=?module=resume&act=editdownload&id=$r[id_download]>
  <center><img src='img/edit.png' class='ttLT'' title='Edit File'></center></a>
  </td>
   
  <td width='8%'>
  <a href=javascript:confirmdelete('$aksi?module=resume&act=hapus&id=$r[id_download]&namafile=$r[nama_file]')>
  ...
  $jmldata=mysql_num_rows(mysql_query("SELECT * FROM resume"));
  ...
  <form method=POST action='$aksi?module=resume&act=input' enctype='multipart/form-data'>
  ...
  $tampil=mysql_query("SELECT * FROM kategoriresume ORDER BY nama_kategoridownload");
  while($r=mysql_fetch_array($tampil)){
  ...
  case "editdownload":
   if ($_SESSION['leveluser']=='admin'){
  $edit = mysql_query("SELECT * FROM resume WHERE id_download='$_GET[id]'");}
    else{
		$edit = mysql_query("SELECT * FROM resume WHERE id_download='$_GET[id]' AND username='$_SESSION[namauser]'");}
  ...
  <div class='head'>
  <div class='isw-documents'></div>
  <h1>Edit File Resume</h1>
  <div class='clear'></div>
  </div>
					
  <div class='block-fluid'>             
  <form method=POST enctype='multipart/form-data' action=$aksi?module=resume&act=update>
