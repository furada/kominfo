...
session_start();
 if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])){  
  ...
  $aksi="modul/mod_imb/aksi_imb.php";
  switch($_GET[act]){
  // Tampil profil
  default:
  ...
  <strong>Anda berhasil</strong> meng-update Simulasi IMB.
  </div>";}
  
  $sql  = mysql_query("SELECT * FROM profil where id_profil=6");
  ...
  <form method=POST enctype='multipart/form-data' action=$aksi?module=imb&act=update>
  ...
  <img src='../img_teraskreasi/small_$r[gambar]' width=200 class='ttLT'' title='Perbesar Gambar'></a></div>
  <div class='clear'></div>
  </div> "; 
