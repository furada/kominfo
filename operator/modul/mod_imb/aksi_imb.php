...
if ($module=='profil' AND $act=='hapus'){
  $data=mysql_fetch_array(mysql_query("SELECT gambar FROM profil WHERE id_profil='$_GET[id]'"));
  if ($data['gambar']!=''){
...
// Hapus terpilih
elseif($module=='imb' AND $act=='hapussemua'){
if(isset($_POST['cek'])){
foreach($_POST['cek'] as $cek => $num){
$data=mysql_fetch_array(mysql_query("SELECT gambar FROM profil WHERE id_profil=$num"));
  if ($data['gambar']!=''){
     unlink("../../../img_teraskreasi/$data[gambar]");   
     unlink("../../../img_teraskreasi/small_$data[gambar]");  
} 
mysql_query("DELETE FROM profil WHERE id_profil=$num");
  header('location:../../media.php?module='.$module.'&msg=delete');
}
} else {
  header('location:../../media.php?module='.$module.'&msg=delete');
...
// Input profil
elseif ($module=='imb' AND $act=='input'){
  ...
  if (!empty($lokasi_file)){
    ...
    echo "<script>window.alert('Upload Gagal, Pastikan File yang di Upload bertipe *.JPG');
        window.location=('../../media.php?module=imb)</script>";
    }
    else{
...
// Update profil
elseif ($module=='imb' AND $act=='update'){
  $lokasi_file    = $_FILES['fupload']['tmp_name'];
  $tipe_file      = $_FILES['fupload']['type'];
  $nama_file      = $_FILES['fupload']['name'];
  $acak           = rand(1,99);
